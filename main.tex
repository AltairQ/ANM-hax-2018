...
$ \langle f + h, g \rangle = \langle f, g \rangle + \langle h, g \rangle$ $p(x)$ - funkcja wagowa\\

$\langle f, g \rangle = \int_{a}^{b} p(x)f(x)g(x) dx$, ${\langle f, g \rangle}_{D} = \sum_{k=0}^n f(x_k)g(x_k) $
(dla ustalonego $x_k$)
